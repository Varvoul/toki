...
RUN echo "STEP1_COPY_TOOLS:PASS" > /app/results.txt

ENV COMPOSER_HOME="/tmp/composer"
ENV COMPOSER_MEMORY_LIMIT=-1

RUN (install-php-extensions intl mbstring mongodb-stable redis opcache sockets pcntl 2>&1 | tail -3 && echo "STEP2_PHP_EXTS:PASS" >> /app/results.txt) || (echo "STEP2_PHP_EXTS:FAIL" >> /app/results.txt && false)
...
RUN (apt-get update > /dev/null 2>&1 && apt-get install -y --no-install-recommends openssl git wget unzip curl > /dev/null 2>&1 \
    ...
    && echo "STEP3_SYSTEM:PASS" >> /app/results.txt) || (echo "STEP3_SYSTEM:FAIL" >> /app/results.txt && false)
...
RUN (adduser --disabled-password --shell "/sbin/nologin" --home "/nonexistent" --no-create-home --uid "10001" --gecos "" "jikanapi" \
    ...
    && echo "STEP4_USER:PASS" >> /app/results.txt) || (echo "STEP4_USER:FAIL" >> /app/results.txt && false)

COPY --chown=jikanapi:jikanapi ./composer.json ./composer.lock /app/
WORKDIR /app
RUN (composer install --no-dev --no-cache --no-ansi --no-autoloader --no-scripts --prefer-dist 2>&1 | tail -10 >> /app/results.txt && echo "STEP5_COMPOSER:PASS" >> /app/results.txt) || (echo "STEP5_COMPOSER:FAIL" >> /app/results.txt && false)
