...
RUN echo "STEP1_TOOLS:PASS" > /app/build.log

RUN (install-php-extensions intl mbstring mongodb-stable redis opcache sockets pcntl 2>&1 | tail -5 && echo "STEP2_EXTS:PASS" >> /app/build.log) || (echo "STEP2_EXTS:FAIL" >> /app/build.log; true)
...
RUN (apt-get update > /dev/null 2>&1 && apt-get install -y --no-install-recommends openssl git wget unzip curl > /dev/null 2>&1 \
    ...
    && echo "STEP3_SYSTEM:PASS" >> /app/build.log) || (echo "STEP3_SYSTEM:FAIL" >> /app/build.log; true)
...
RUN (adduser --disabled-password --shell "/sbin/nologin" --home "/nonexistent" --no-create-home --uid "10001" --gecos "" "jikanapi" \
    ...
    && echo "STEP4_USER:PASS" >> /app/build.log) || (echo "STEP4_USER:FAIL" >> /app/build.log; true)

COPY --chown=jikanapi:jikanapi ./composer.json ./composer.lock /app/
WORKDIR /app

RUN (composer install --no-dev --no-cache --no-ansi --no-autoloader --no-scripts --prefer-dist 2>&1 | tail -10 >> /app/build.log && echo "STEP5_COMPOSER:PASS" >> /app/build.log) || (echo "STEP5_COMPOSER:FAIL" >> /app/build.log; true)
